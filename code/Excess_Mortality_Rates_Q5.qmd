...
dat2 |>
  ...
  labs(x = 'Time (Calendar Time)', 
       y = 'Rates (per 100000)',
       title = paste('Wave 1: Rates of Excess deaths, & total deaths in U.S. for Region'), 
       color = 'Region') + 
...
dat2 |>
  ...
  labs(x = 'Time (Calendar Time)', 
       y = 'Rates (per 100000)',
       title = 'Wave 2: Rates of Excess deaths, & total deaths in U.S.',
       color = 'Region') + 
  theme(axis.text.x = element_text(vjust = 0.75, size = 6, angle = 45),
        legend.key.size = unit(0.25, 'cm'))
```
...
dat2 |>
  ...
  filter(date >= make_date(2021, 8, 1) & date <= make_date(2021, 11, 31)) |>
  ggplot(aes(x = date, y = rate, color = state)) + 
  geom_line(aes(group = state)) + # creates smooth lines
  facet_grid(type ~ region, scales = 'free_y') + # stack/scale axis
  labs(x = 'Time (Calendar Time)', 
       y = 'Rates (per 100000)',
       title = 'Wave 3: Rates of Excess deaths, & total deaths in U.S.',
       color = 'Region') + 
  theme(axis.text.x = element_text(vjust = 0.75, size = 6, angle = 45),
        legend.key.size = unit(0.25, 'cm'))
```
